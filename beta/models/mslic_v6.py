...
class MSLIC_V6(CompressionModel):
    def __init__(self, config, **kwargs):
        ...
        self.entropy_parameters_x1 = nn.ModuleList(
            EntropyParameters(in_dim=slice_ch * slice_num[0] * 2 + slice_ch * i + slice_ch * slice_num[0], out_dim=slice_ch * 2)
            if i else EntropyParameters(in_dim=slice_ch * slice_num[0] * 2 + slice_ch * slice_num[0], out_dim=slice_ch * 2)
            for i in range(slice_num[0])
        )
        self.entropy_parameters_x2 = nn.ModuleList(
            EntropyParameters(in_dim=slice_ch * slice_num[1] * 2 + slice_ch * i + slice_ch * slice_num[1], out_dim=slice_ch * 2)
            if i else EntropyParameters(in_dim=slice_ch * slice_num[1] * 2 + slice_ch * slice_num[1], out_dim=slice_ch * 2)
    ...
    def forward(self, x):
        ...
        for slice_index, y_slice in enumerate(y4_slices):
            if slice_index == 0:
                ...
                params_support = torch.cat([hyper_params_4] + y4_hat_slices, dim=1)
