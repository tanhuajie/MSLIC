...
class MLICPlusPlus(CompressionModel):
    def __init__(self, config, **kwargs):
        ...
        self.channel_context = nn.ModuleList(
            ChannelContext(in_dim=slice_ch * i, out_dim=slice_ch * 4) if i else None
    ...
    def forward(self, x):
        ...
        z_hat = ste_round(z - z_offset) + z_offset # [B, N, H // 64, W // 64]

        # Hyper-parameters
        hyper_params = self.h_s(z_hat) # [B, 2 * M, H // 16, W // 16]
        hyper_scales, hyper_means = hyper_params.chunk(2, 1) # [B, M, H // 16, W // 16]

        y_slices = y.chunk(self.slice_num, dim=1) # [B, slice_ch, H // 16, W // 16]
        y_hat_slices = []
        y_likelihoods = []
        for idx, y_slice in enumerate(y_slices):
            slice_anchor, slice_nonanchor = ckbd_split(y_slice) # [B, slice_ch, H // 16, W // 16]
            if idx == 0:
                # Anchor
                params_anchor = self.entropy_parameters_anchor[idx](hyper_params) # [B, 2 * slice_ch, H // 16, W // 16]
                scales_anchor, means_anchor = params_anchor.chunk(2, 1) # [B, slice_ch, H // 16, W // 16]
                # split means and scales of anchor
                scales_anchor = ckbd_anchor(scales_anchor)  # [B, slice_ch, H // 16, W // 16]
                means_anchor = ckbd_anchor(means_anchor)    # [B, slice_ch, H // 16, W // 16]
                # round anchor
                slice_anchor = ste_round(slice_anchor - means_anchor) + means_anchor # [B, slice_ch, H // 16, W // 16]
                # predict residuals cause by round
                lrp_anchor = self.lrp_anchor[idx](torch.cat(([hyper_means] + y_hat_slices + [slice_anchor]), dim=1)) # [B, M + idx * X + slice_ch, H // 16, W // 16] --> [B, slice_ch, H // 16, W // 16]
                slice_anchor = slice_anchor + ckbd_anchor(lrp_anchor) # [B, slice_ch, H // 16, W // 16]
                # Non-anchor
                # local_ctx: [B, H, W, 2 * C]
                local_ctx = self.local_context[idx](slice_anchor) # [B, 2 * slice_ch, H // 16, W // 16]
                params_nonanchor = self.entropy_parameters_nonanchor[idx](torch.cat([local_ctx, hyper_params], dim=1)) # [B, 2 * slice_ch + 2 * M, H // 16, W // 16] --> [B, 2 * slice_ch, H // 16, W // 16]
                scales_nonanchor, means_nonanchor = params_nonanchor.chunk(2, 1) # [B, slice_ch, H // 16, W // 16]
                # split means and scales of nonanchor
                scales_nonanchor = ckbd_nonanchor(scales_nonanchor) # [B, slice_ch, H // 16, W // 16]
                means_nonanchor = ckbd_nonanchor(means_nonanchor)   # [B, slice_ch, H // 16, W // 16]
                # merge means and scales of anchor and nonanchor
                scales_slice = ckbd_merge(scales_anchor, scales_nonanchor)  # [B, slice_ch, H // 16, W // 16]
                means_slice = ckbd_merge(means_anchor, means_nonanchor)     # [B, slice_ch, H // 16, W // 16]
                _, y_slice_likelihoods = self.gaussian_conditional(y_slice, scales_slice, means_slice)  # [B, slice_ch, H // 16, W // 16]
                # round slice_nonanchor
                slice_nonanchor = ste_round(slice_nonanchor - means_nonanchor) + means_nonanchor # [B, slice_ch, H // 16, W // 16]
                y_hat_slice = slice_anchor + slice_nonanchor  # [B, slice_ch, H // 16, W // 16]
                # predict residuals cause by round
