...
def test_one_epoch(epoch, test_dataloader, model, criterion, logger_val, tb_logger, save_dir, save_pic):
    ...
    mse_loss_ds1 = AverageMeter()
    mse_loss_ds2 = AverageMeter()
    mse_loss_ds4 = AverageMeter()

    ms_ssim_loss_ds1 = AverageMeter()
    ms_ssim_loss_ds2 = AverageMeter()
    ms_ssim_loss_ds4 = AverageMeter()

    psnr_ds2 = AverageMeter()
    psnr_ds4 = AverageMeter()

    ms_ssim_ds2 = AverageMeter()
    ms_ssim_ds4 = AverageMeter()
    ...
    with torch.no_grad():
        for i, d in enumerate(test_dataloader):
            ...
            if out_criterion["mse_loss"] is not None:
                ...
                mse_loss_ds1.update(out_criterion["mse_loss_ds1"])
                mse_loss_ds2.update(out_criterion["mse_loss_ds2"])
                mse_loss_ds4.update(out_criterion["mse_loss_ds4"])
            if out_criterion["ms_ssim_loss"] is not None:
                ms_ssim_loss.update(out_criterion["ms_ssim_loss"])
                ms_ssim_loss_ds1.update(out_criterion["ms_ssim_loss_ds1"])
                ms_ssim_loss_ds2.update(out_criterion["ms_ssim_loss_ds2"])
                ms_ssim_loss_ds4.update(out_criterion["ms_ssim_loss_ds4"])

            rec = torch2img(out_net['x_hat'])
            rec_ds2 = torch2img(out_net['x_hat_ds2'])
            rec_ds4 = torch2img(out_net['x_hat_ds4'])
            img = torch2img(d)
            ...
            p_ds2, m_ds2 = compute_metrics(rec_ds2, img)
            psnr_ds2.update(p_ds2)
            ms_ssim_ds2.update(m_ds2)

            p_ds4, m_ds4 = compute_metrics(rec_ds4, img)
            psnr_ds4.update(p_ds4)
            ms_ssim_ds4.update(m_ds4)

            if save_pic == True:
                if not os.path.exists(save_dir):
                    os.makedirs(save_dir)
                rec.save(os.path.join(save_dir, '%03d_rec.png' % i))
                rec_ds2.save(os.path.join(save_dir, '%03d_rec_ds2.png' % i))
                rec_ds4.save(os.path.join(save_dir, '%03d_rec_ds4.png' % i))
...
def decompress_one_image(model, stream_path, img_name):
    ...
    x_hat = out["x_hat"]
    x_hat_ds2 = out["x_hat_ds2"]
    x_hat_ds4 = out["x_hat_ds4"]
    x_hat = x_hat[:, :, 0 : original_size[0], 0 : original_size[1]]
    x_hat_ds2 = x_hat_ds2[:, :, 0 : original_size[0], 0 : original_size[1]]
    x_hat_ds4 = x_hat_ds4[:, :, 0 : original_size[0], 0 : original_size[1]]
    cost_time = out["cost_time"]
    return x_hat, x_hat_ds2, x_hat_ds4, cost_time
...
def test_model(test_dataloader, net, logger_test, save_dir, epoch):
    ...
    avg_psnr_ds2 = AverageMeter()
    avg_psnr_ds4 = AverageMeter()

    avg_ms_ssim_ds2 = AverageMeter()
    avg_ms_ssim_ds4 = AverageMeter()
    ...
    with torch.no_grad():
        for i, img in enumerate(test_dataloader):
            ...
            # # avoid resolution leakage
            # net.update_resolutions(16, 16)
            x_hat, x_hat_ds2, x_hat_ds4, dec_time = decompress_one_image(model=net, stream_path=save_dir, img_name=str(i))
            rec = torch2img(x_hat)
            rec_ds2 = torch2img(x_hat_ds2)
            rec_ds4 = torch2img(x_hat_ds4)
            img = torch2img(img)
            ...
            p_ds2, m_ds2 = compute_metrics(rec_ds2, img)
            avg_psnr_ds2.update(p_ds2)
            avg_ms_ssim_ds2.update(m_ds2)

            p_ds4, m_ds4 = compute_metrics(rec_ds4, img)
            avg_psnr_ds4.update(p_ds4)
            avg_ms_ssim_ds4.update(m_ds4)
            ...
            logger_test.info(
                ...
                f"PSNR_DS4: {p_ds4:.4f} | "
                f"MS-SSIM: {m:.4f} | "                
                f"MS-SSIM_DS2: {m_ds2:.4f} | "                
                f"MS-SSIM_DS4: {m_ds4:.4f} | "
    ...
    logger_test.info(
        ...
        f"Avg PSNR_DS2: {avg_psnr_ds2.avg:.4f} | "
        f"Avg PSNR_DS4: {avg_psnr_ds4.avg:.4f} | "
        f"Avg MS-SSIM: {avg_ms_ssim.avg:.4f} | "
        f"Avg MS-SSIM_DS2: {avg_ms_ssim_ds2.avg:.4f} | "
        f"Avg MS-SSIM_DS4: {avg_ms_ssim_ds4.avg:.4f} | "
