...
class MSLIC_V5(CompressionModel):
    def __init__(self, config, **kwargs):
        ...
        self.multi_channel_context_x2_to_x1 = ScaleX2Context_MS(in_dim=slice_ch * slice_num[1], out_dim=slice_ch * 4)
        self.multi_channel_context_x4_to_x1 = ScaleX4Context_MS(in_dim=slice_ch * slice_num[2], out_dim=slice_ch * 2)
        self.multi_channel_context_x4_to_x2 = ScaleX2Context_MS(in_dim=slice_ch * slice_num[2], out_dim=slice_ch * 4)

        # Entropy Parameters
        self.entropy_parameters_x1 = nn.ModuleList(
            EntropyParameters(in_dim=slice_ch * slice_num[0] * 2 + slice_ch * (i + i * 2) + slice_ch * 6, out_dim=slice_ch * 2)
            if i else EntropyParameters(in_dim=slice_ch * slice_num[0] * 2 + slice_ch * 6, out_dim=slice_ch * 2)
            for i in range(slice_num[0])
        )
        self.entropy_parameters_x2 = nn.ModuleList(
            EntropyParameters(in_dim=slice_ch * slice_num[1] * 2 + slice_ch * (i + i) + slice_ch * 4, out_dim=slice_ch * 2)
            if i else EntropyParameters(in_dim=slice_ch * slice_num[1] * 2 + slice_ch * 4, out_dim=slice_ch * 2)
            for i in range(slice_num[1])
        )
    ...
    def forward(self, x):
        ...
        for slice_index, y_slice in enumerate(y4_slices):
            if slice_index == 0:
                ...
                channel_ctx = torch.cat(y4_hat_slices, dim=1)
        ...
        chn_ctx_x4_to_x2 = self.multi_channel_context_x4_to_x2(torch.cat(y4_hat_slices, dim=1))
        chn_ctx_x4_to_x1 = self.multi_channel_context_x4_to_x1(torch.cat(y4_hat_slices, dim=1))
        ...
        for slice_index, y_slice in enumerate(y2_slices):
            if slice_index == 0:
                ...
                channel_ctx = torch.cat(y2_hat_slices, dim=1)
                params_support = torch.cat([hyper_params_2, scale_attn, channel_ctx, chn_ctx_x4_to_x2], dim=1)
        ...
        chn_ctx_x2_to_x1 = self.multi_channel_context_x2_to_x1(torch.cat(y2_hat_slices, dim=1))
        ...
        for slice_index, y_slice in enumerate(y1_slices):
            if slice_index == 0:
                params_support = torch.cat([hyper_params_1, chn_ctx_x2_to_x1, chn_ctx_x4_to_x1], dim=1)
            else:
                ...
                channel_ctx = torch.cat(y1_hat_slices, dim=1)
                params_support = torch.cat([hyper_params_1, scale_attn_1, scale_attn_2, channel_ctx, chn_ctx_x2_to_x1, chn_ctx_x4_to_x1], dim=1)
